...
def create_user(name:str, user_name: str, password: str, db: Session):
    try:
        user = db.query(User.user_name).filter(User.user_name == user_name).first()
...
def log_in(username,password, db : Session):
    try:
        ...
        if not db_user:
            raise HTTPException(status_code= status.HTTP_400_BAD_REQUEST, detail="User not exist, Please Sign-up")
