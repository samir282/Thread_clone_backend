...
@post_router.post("/profile_timeline", status_code= status.HTTP_200_OK)
def profile_timeline(page: int = Query(1, ge=1),
                  limit: int = Query(10, ge=1, le=100),
                  current_user : User = Depends(get_current_user),
                  db : Session = Depends(get_db)):
    return get_profile_timeline(current_user.username, page, limit, db)
